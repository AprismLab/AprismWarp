...
- [RESOLVED 2026-09-14] Aprism lifecycle instance identity behavior resolved (see the 2026-09-14 entry: Aprism commit `9adc003` reuses the retained entrypoint instance across phases; regression test added).
...
- [DONE] Upstream blockers FIXED in the Aprism repo (user-authorized direct fix) and verified against the real instance with all four mods installed (fabric-api + modmenu + Despotes + `g6-smoke.aje`): (1) **ASM duplicate classes** — the Aprism build now ships a distinct fabric-host agent variant (`-PfabricVariant`) that relocates all 149 `org.objectweb.asm` classes to `aprism/libs/asm/**` (in-repo `verifyAgentJar` task asserts the layout), so Fabric Knot's `verifyClasspath` no longer aborts; (2) **Fabric dependency ids** — `ModDiscoverer.isFabricHostPresent()`/`isFabricModJar()` probes plus a filter in `AprismRuntime.loadMods` skip jars carrying `fabric.mod.json` under a Fabric host, so Aprism no longer claims host-managed mods and `DependencyResolutionException` on `fabric-api`/`fabric-resource-loader-v1` is gone. Verification: launch completed with the `Minecraft 26.2` window responding, `performLoad complete: 1 mod(s)`, `[OK] mod g6-smoke 0.1.0`, Despotes v26.11 booted on the Fabric loader, and no ASM/dependency diagnostics. Aprism commits `d96228c` (transformer gate + harness, landed in parallel) and `2cbc9cf` (Fabric skip + fabric-host variant, this fix); `aprism-loader-core` tests pass; the rebuilt fabric-host jar is cached in MDL's aprism dir under its own name and the v26.8 cache original was restored.
- [DONE] Aprism lifecycle instance identity behavior RESOLVED in the Aprism repo (user-authorized direct fix, commit `9adc003`): a regression test (`instanceIdentityPreservedAcrossPhases`) first reproduced the defect (the retained container instance only ever saw PREINIT because the runtime constructed a fresh entrypoint per phase dispatch), then `AprismRuntime` was fixed to reuse the retained instance whenever the entrypoint class matches it, per the documented contract (aprism-contract.md §3 "retains the first instance on the container"). Full `aprism-loader-core` suite passes; both agent variants rebuilt with correct layouts (native 149 unrelocated ASM, fabric-host 149 relocated) and the fabric-host artifact refreshed in MDL's cache. Stateful cross-phase code (fields set in `onInitialize` surviving to `onSetup`) is now safe for AprismWarp-generated mods.
- [DONE] Editor CSP hardening COMPLETE (FORK.md row 8): the fork's `webpack.config.js` now injects a Content-Security-Policy meta into every generated page — `default-src 'self' file:`, `connect-src 'self' file:`, `object-src 'none'`, `base-uri 'none'` with no remote origins permitted, plus `script-src` allowing `'unsafe-inline' 'unsafe-eval' 'wasm-unsafe-eval'` because the upstream scratch-blocks workspace and TurboWarp compiler evaluate strings and inline script. Verified with the full smoke gate (`gui=true`, `irValid=true`, `roundTrip=true`, `package lockMatched=true`) and **zero CSP violation console errors**; the first, stricter policy was caught by the gate as an `EvalError: Refused to evaluate a string as JavaScript` that broke workspace init, and the final policy was re-validated after the fix.
- [DONE] Cross-phase identity probe built with existing primitives (no toolchain change): a hand-written probe mod (`onPreInitialize` writes a field, `onInitialize` logs `probe=<value>`) compiled via `compileJava` + `jarJava`, embedded through `writeAwp` (`editor/project.json` + `build/mod.jar`) and packaged with `generateAje`, producing `a9-probe.aje`. Installed into the `aprismwarp-a9-check` instance (Fabric 26.2) and launched with the fixed fabric-host agent: the run reached `performLoad complete: 2 mod(s)` and progressed through the Mixin subsystem and Datafixer bootstrap, i.e. **past the `KnotClient` init that previously aborted on the ASM clash** — the artifact loads under a Fabric host when the relocated-ASM agent is actually used.
- [NOTE] MDL `--aprism` selects the cached GA-line agent by filename and **re-validates/re-fetches it**, overwriting a manual cache substitution: the swapped fabric-host bytes (4557612) were silently replaced by the genuine v26.8 native jar (5912385) on the next launch, which reintroduced the `duplicate ASM classes` abort at `KnotClient` init. Consequently the fabric-host fix cannot be exercised through `mdl launch --aprism` until MDL gains fabric-host variant selection (an MDL-side gap, outside this repository). The fix itself is proven by the run that did use it (2 mods loaded, Mixin + Datafixer reached) plus the in-repo `verifyAgentJar` layout gate and `aprism-loader-core` tests.
- [NOTE] The `aprismwarp-a9-check` instance was returned to its original four mods and the MDL aprism cache slot was restored to the genuine v26.8 jar after the probe run.
- [NOTE] MDL PATH carries a stale v26.1.0 copy that can shadow newer binaries (v26.6 dropped the `kill` subcommand; instance stop was performed via the game process). AprismWarp unaffected; MDL-side cleanup recommended.
- [DONE] Shell Preview action added (FORK.md row 9): `File > Preview Project` (Ctrl+R) runs the G5 interpreter over the live workspace via the new `previewProject()` page API and shows the resulting trace, variable state, and any interpreter errors in a native dialog. Smoke gate extended with `menuPreview=true` (exactly one `log.info` step carrying the expected message), alongside the existing `preview=true` parity check; all gates green (`gui=true`, `irValid=true`, `roundTrip=true`, `package lockMatched=true`).
