...
| 9 | `src/lib/aprismwarp-blocks.js` (G5 extension of row 2): `previewProject()` page API that extracts the live workspace as IR and runs `previewIr` over it | gives the shell a user-facing preview action (`File > Preview Project`, Ctrl+R) without touching upstream menu code; the Electron main process renders the returned trace and variable state in a dialog | same file |
